...
<h2><spring:message code="mohappointment.appointment.service.provider.management"/></h2>
<br/>
<a href="serviceProvider.form"><spring:message code="mohappointment.appointment.service.provider.add"/></a>
<br/><br/>
...
<script type="text/javascript">
	...
	function showEditWindow(pId){
		$app("#divWindow").html("<div id='dialog' style='font-size: 0.9em;' title='<spring:message code='mohappointment.appointment.service.provider.form'/>'><div id='result'>"+$app('#editWindow').html()+"</div></div>");
		$app("#dialog").dialog({
			zIndex: 980,
			bgiframe: true,
			height: 220,
...
<div class="searchParameterBox box">
	<div class="list_container" style="width: 99%">
		<div class="list_title">
			<div class="list_title_msg"><spring:message code="mohappointment.appointment.service.provider.current"/></div>
		...
		<table class="list_data">
			...
			<c:forEach items="${serviceProviders}" var="sp" varStatus="status">
				<tr>
					...
					<!-- <td class="rowValue ${status.count%2!=0?'even':''}"><input type="button" id="providerServiceEdit" value="<spring:message code='mohappointment.general.edit'/>"></td> -->
					<td class="rowValue ${status.count%2!=0?'even':''}"><a href="serviceProvider.form?editSP=true&editServiceProviderId=${sp.serviceProviderId}"><spring:message code='mohappointment.general.edit'/></a>
					 | <a id="deleteCtrl_${sp.serviceProviderId}" onclick="submitData(${sp.serviceProviderId});" href="#"><spring:message code='mohappointment.general.delete'/></a></td>	
				</tr>
			</c:forEach>
		</table>
